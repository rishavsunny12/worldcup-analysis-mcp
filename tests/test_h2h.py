...
async def test_get_h2h_happy_path():
    raw = load_fixture("api_football_h2h_brazil_england.json")
    with patch("tools.head_to_head.api_football") as mock_api, \
         patch("tools.head_to_head.cache") as mock_cache:
        mock_api.get_h2h = AsyncMock(return_value=raw)
